...
echo %DATE% %TIME% -----Start 02.sf_retrieve_after-----
...
REM retrieve start
Call sf project retrieve start --api-version %SoapApiVersion% --manifest %batch_dir%\retrievemanifest\package.xml --target-metadata-dir %batch_dir%\02.retrieved_after --unzip --target-org %SfUserName% --zip-file-name %today%_%currenttime%_number.zip
if %errorlevel% neq 0 (
